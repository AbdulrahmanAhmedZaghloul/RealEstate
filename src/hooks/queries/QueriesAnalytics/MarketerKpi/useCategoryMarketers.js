...
const fetchCategoryPerformance = async (token, timeFrame, month, year) => {
    ...
    const { data } = await axiosInstance.get(`marketer/kpi/listings/category-performance?${params.toString()}`, {
        headers: { Authorization: `Bearer ${token}` },
    });
    return data;
};

export const useCategoryMarketers = (timeFrame = "yearly", month = "", year = "") => {
    const { user } = useAuth();

    return useQuery({
        queryKey: ['categoryPerformanceMarketers', timeFrame, month, year],
