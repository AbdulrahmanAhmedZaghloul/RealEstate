...
const fetchTimeOnMarket = async (token, timeFrame, month, year) => {
    ...
    const { data } = await axiosInstance.get(`kpi/time-on-market?${params.toString()}`, {
        headers: { Authorization: `Bearer ${token}` },
    });
    return data;
};

export const useTimeOnMarket = (timeFrame = "yearly", month = "", year = "") => {
    const { user } = useAuth();

    return useQuery({
        queryKey: ['timeOnMarket', timeFrame, month, year],
