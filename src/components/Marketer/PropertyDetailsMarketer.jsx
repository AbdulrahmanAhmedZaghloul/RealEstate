...
function PropertyDetailsMarketer() {
  ...
  const handleUpdateProperty = async (updatedData) => {
    try {
      await axiosInstance.put(`/marketer/listings/${id}`, updatedData, {
        headers: { Authorization: `Bearer ${user.token}` },
      });
  ...
  const handleDeleteProperty = async () => {
    ...
    try {
      ...
      navigate("/dashboard-Marketer/PropertiesMarketer");
      queryClient.invalidateQueries(['listingsRealEstate-Marketer']);
  ...
  const handleUpdateListing = async (newStatus, reason) => {
    setLoading(true);
    try {
      await axiosInstance.post(`listings/${id}/status`,
...
export default PropertyDetailsMarketer;
